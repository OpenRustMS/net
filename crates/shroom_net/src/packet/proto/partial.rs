    ...
    FlagData::Flags: EncodePacket + std::fmt::Debug,
{
    ...
    fn encode_packet<T: bytes::BufMut>(&self, pw: &mut PacketWriter<T>) -> NetResult<()> {
        let flags = self.data.get_flags();
...
macro_rules! partial_data {
    ($name:ident, $partial_name:ident, $partial_ty:ty, $($stat_name:ident($stat_ty:ty) => $stat_ix:expr),* $(,)?) => {
        ...
        paste::paste! {
            ...
            impl <'de> $crate::packet::proto::partial::PartialData<'de> for [<$name Partial>] {
                ...
                fn partial_encode_packet<Buf: bytes::BufMut>(&self, _flag: Self::Flags, pw: &mut $crate::PacketWriter<Buf>) -> $crate::NetResult<()> {
                ...
                fn partial_decode_packet(flag: Self::Flags, pr: &mut $crate::PacketReader<'de>) -> $crate::NetResult<Self> {
                    use $crate::packet::proto::conditional::{CondOption, PacketConditional};
                    Ok(Self {
            ...
            impl $crate::EncodePacket for [<$name All>] {
                ...
                fn encode_packet<T: bytes::BufMut>(&self, pw: &mut $crate::PacketWriter<T>) -> $crate::NetResult<()> {
                    $(self.[<$stat_name:lower>].encode_packet(pw)?;)*
                    Ok(())
                }
            }
            ...
            impl $crate::packet::proto::PacketWrapped for [<$partial_name All>] {
