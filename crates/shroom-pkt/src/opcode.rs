use crate::{error::Error, util::SizeHint, DecodePacket, EncodePacket, PacketResult};
    ...
    T: EncodePacket,
{
    ...
    fn encode_packet<B: bytes::BufMut>(&self, pw: &mut crate::PacketWriter<B>) -> PacketResult<()> {
...
where
    T: DecodePacket<'de>,
{
    fn decode_packet(pr: &mut crate::PacketReader<'de>) -> PacketResult<Self> {
        Ok(Self(T::decode_packet(pr)?))
    }
}

/// Helper macro to easily implment `HasOpcode` for a packet
/// Example ```packet_opcode!(Handshake, SendOpcode::Ping);```
#[macro_export]
