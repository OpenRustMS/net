...
macro_rules! mark_shroom_bitflags {
    ($bits_ty:ty) => {
        impl $crate::PacketWrapped for $bits_ty {
            type Inner = $crate::ShroomBitFlags<$bits_ty>;
...
macro_rules! mark_shroom_packed_struct {
    ($packed_strct_ty:ty) => {
        impl $crate::PacketWrapped for $packed_strct_ty {
            type Inner = $crate::ShroomPackedStruct<$packed_strct_ty>;

            fn packet_into_inner(&self) -> Self::Inner {
                $crate::ShroomPackedStruct(self.clone())
            }
...
mod tests {
    ...
    fn packet_struct() {
        ...
        test_enc_dec(ShroomPackedStruct(0u8));
    }
}
