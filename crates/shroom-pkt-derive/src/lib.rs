...
impl PacketField {
    ...
    pub fn packet_len_expr(&self, field_name: &TokenStream) -> TokenStream {
        if let Some(cond) = self.get_cond() {
            ...
            quote::quote! ( shroom_pkt::PacketConditional::packet_len_cond(&self.#field_name, #cond) )
        } else {
    ...
    pub fn encode_expr(&self, field_name: &TokenStream) -> TokenStream {
        if let Some(cond) = self.get_cond() {
            let cond = cond.self_expr();
            quote::quote! ( shroom_pkt::PacketConditional::encode_packet_cond(&self.#field_name, #cond, pw) )
        } else {
    ...
    pub fn decode_expr(&self, var_ident: &Ident) -> TokenStream {
        ...
        if let Some(cond) = self.get_cond() {
            ...
            quote::quote!( let #var_ident  = <#ty as shroom_pkt::PacketConditional>::decode_packet_cond(#cond, pr) )

            // Call the sized decoder with the given sized expression
        } else if let Some(sz) = self.size.as_ref() {
            quote::quote!( let #var_ident = shroom_pkt::DecodePacketSized::decode_packet_sized(pr, #sz as usize) )
        } else {
...
impl ShroomPacket {
    ...
    fn gen_decode(&self, token_stream: &mut proc_macro2::TokenStream) -> syn::Result<()> {
        ...
        token_stream.extend(quote::quote!(impl #de_impl_generics  shroom_pkt::DecodePacket<#de_lifetime> for #struct_name #ty_generics #de_where_clause  {
            fn decode_packet(pr: &mut shroom_pkt::PacketReader<#de_lifetime>) -> shroom_pkt::PacketResult<Self> {
                #(#dec_var)*
                Ok(#struct_name {
                    #(#struct_dec_fields)*
                })
    ...
    fn gen_encode(&self, token_stream: &mut proc_macro2::TokenStream) -> syn::Result<()> {
        ...
        token_stream.extend(quote::quote!(impl #impl_generics shroom_pkt::EncodePacket for #struct_name #ty_generics #where_clause {
            fn encode_packet<B: bytes::BufMut>(&self, pw: &mut shroom_pkt::PacketWriter<B>) -> shroom_pkt::PacketResult<()> {
                #(#struct_enc_fields)*
                Ok(())
            }
