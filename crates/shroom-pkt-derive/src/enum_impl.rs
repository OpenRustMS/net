...
impl ShroomEnumVariant {
    fn gen_packet_len(&self) -> proc_macro2::TokenStream {
    ...
    fn gen_decode(&self) -> proc_macro2::TokenStream {
        ...
        let decode = self.fields.iter().enumerate().map(|(i, field)| {
            let ident = format_ident!("_{i}");
            let ty = &field.ty;
            quote::quote! { let #ident = <#ty>::decode_packet(pr)?; }
        });
    ...
    fn gen_encode(&self) -> proc_macro2::TokenStream {
        ...
        let field_encode = self.fields.iter().enumerate().map(|(i, _)| {
            let ident = format_ident!("_{i}");
            quote::quote! { #ident.encode_packet(pw)?; }
        });
...
impl ToTokens for ShroomPacketEnum {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        let len_fields = enum_.iter().map(|v| v.gen_packet_len());
        ...
        tokens.extend(quote::quote!(
            impl shroom_pkt::EncodePacket for #ident {
                ...
                fn packet_len(&self) -> usize {
                    unsafe { *<*const _>::from(self).cast::<#repr_ty>() }.packet_len() +
                    match self {
                        #(#len_fields)*
                    }
                }
                ...
                fn encode_packet<B: bytes::BufMut>(&self, pw: &mut shroom_pkt::PacketWriter<B>) ->  shroom_pkt::PacketResult<()> {
                    unsafe { *<*const _>::from(self).cast::<#repr_ty>() }.encode_packet(pw)?;
                    match self {
                        #(#enc_fields)*
                    }
                    Ok(())
            ...
            impl<'de> shroom_pkt::DecodePacket<'de> for #ident {
                fn decode_packet(pr: &mut shroom_pkt::PacketReader<'de>) -> shroom_pkt::PacketResult<Self> {
                    let disc = <#repr_ty>::decode_packet(pr)?;
                    Ok(match disc {
                        #(#dec_fields)*
