...
use shroom_pkt::opcode::HasOpcode;
use shroom_pkt::{EncodePacket, PacketWriter, ShroomPacketData};
use std::hash::Hash;
use std::sync::RwLock;
...
impl<Key: Hash + Eq> SessionSet<Key> {
    ...
    pub fn send_packet_to(&self, rx_key: Key, pkt: ShroomPacketData) -> anyhow::Result<()> {
        self.0
            .read()
    ...
    pub fn broadcast_packet(&self, pkt: ShroomPacketData, src: Key) -> anyhow::Result<()> {
