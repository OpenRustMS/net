...
use crate::{PacketWriter, PacketReader, NetResult};
...
where
    &'a T: EncodePacket,
{
    fn encode_packet<Buf: BufMut>(&self, pw: &mut PacketWriter<Buf>) -> NetResult<()> {
...
mod tests {
    ...
    use crate::{proto::DecodePacket, PacketWriter, PacketReader, NetResult};
    ...
    impl TracingEncodePacket for A {
        ...
        /*fn traac<T: bytes::BufMut>(
            &self,
            pw: &mut crate::packet::PacketWriter<T>,
        ) -> crate::NetResult<()> {
            (
                self.one,
                self.two,
